...
const { generateTOTPSecret, generateQRCode, verifyTOTP, generateBackupCodes, hashBackupCode, verifyBackupCode, markBackupCodeAsUsed, generateReferralCode } = require('../utils/auth');
...
async function login(req, res) {
  try {
    ...
    if (!user) {
      auth.loginFailure(phoneNumber, req.ip, req.headers['user-agent'], 'user_not_found');
    ...
    if (!isValidPassword) {
      ...
      auth.loginFailure(phoneNumber, req.ip, req.headers['user-agent'], 'invalid_password');
      return res.status(401).json({
        success: false,
        message: 'Invalid phone number or password'
      });
...
async function register(req, res) {
  try {
    ...
    if (!sponsorUser) {
      console.log('No sponsor found, looking for any existing user as fallback...');
    ...
    while (!isUnique && attempts < maxAttempts) {
      ...
      const existingReferralCode = await db('users').where({ referral_code: userReferralCode }).first();
    ...
    if (!isUnique) {
      error('Could not generate unique referral code after max attempts', { attempts });
    ...
    if (sponsorUser) {
      ...
      const sponsorPositionData = await Genealogy.getNextAvailablePositionUnderSponsor(sponsorUser.id);
    ...
    if (!position && !parentId) {
      console.log('Creating as root user (no sponsor or no position available)');
      position = null;
      parentId = null;
    }
...
async function verifyBackupCodeEndpoint(req, res) {
  try {
    ...
    if (!isValid) {
      ...
      auth.twoFactorVerificationFailure(userId, 'backup_code', req.ip, 'invalid_code');
...
async function updateProfile(req, res) {
  ...
  try {
    ...
    if (email) {
      ...
      const existingEmailUser = await db('users').where({ email }).andWhereNot({ id: userId }).first();
      if (existingEmailUser) {
        return res.status(409).json({
          success: false,
    ...
    security.info('User profile updated', { userId, updates: Object.keys(updates), ip: req.ip });
...
async function requestPasswordReset(req, res) {
  ...
  try {
    ...
    if (!user) {
      security.info('Password reset requested for unknown email', { email, ip: req.ip });
      return res.json({ success: true, message: genericMessage });
    }

    const token = crypto.randomBytes(32).toString('hex');
    const tokenHash = crypto.createHash('sha256').update(token).digest('hex');
    const ttlMinutes = parseInt(process.env.PASSWORD_RESET_TOKEN_TTL_MINUTES || '60', 10);
    const expiresAt = new Date(Date.now() + ttlMinutes * 60 * 1000);

    await db.transaction(async (trx) => {
      // Clean up existing unused tokens for the user
      await trx('password_reset_tokens').where({ user_id: user.id, used_at: null }).delete();
    ...
    const resetLink = `${resetBaseUrl}${resetBaseUrl.includes('?') ? '&' : '?'}token=${token}`;

    await sendPasswordResetEmail(user.email, user.name || user.phone_number, resetLink);
...
/**
 * Build hierarchical genealogy tree
 */
async function buildGenealogyTree(sponsorId, maxDepth = 10, currentDepth = 0, visited = new Set()) {
  ...
  const tree = await Promise.all(
    downlines.map(async (downline) => ({
      ...
      children: await buildGenealogyTree(downline.id, maxDepth, currentDepth + 1, new Set(visited))
    }))
  );
...
async function getGenealogy(req, res) {
  try {
    ...
    // Build complete genealogy tree starting from current user
    const genealogyTree = await buildGenealogyTree(userId);
